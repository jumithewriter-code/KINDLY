...
export default function Page() {
  ...
  if (childMode) return <ChildMode screen={childScreen} setScreen={setChildScreen} onExit={() => setChildMode(false)} coming={caregiverComing} onRequest={sendRequest} />
  ...
  return (
    <main className="app-shell">
      ...
      <section className="main-content">
        <header className="topbar"><div><span className="mobile-brand">Kindly</span><p className="date-label">{today}</p><h1>{active === 'Home' ? 'Good morning, Jamie' : active}</h1></div><div className="top-actions"><IconButton label="Notifications"><Bell size={20} /></IconButton><div className="avatar large">J</div></div></header>
        {active === 'Home' && <HomeView openChildMode={openChildMode} generated={generated} setGenerated={setGenerated} situation={situation} setSituation={setSituation} format={format} setFormat={setFormat} difficulty={difficulty} setDifficulty={setDifficulty} pendingRequest={pendingRequest} setActive={setActive} />}
        {active === 'Stories' && <StoriesView openChildMode={openChildMode} generated={generated} />}
        {active === 'Requests' && <RequestsView pendingRequest={pendingRequest} coming={caregiverComing} onComing={() => setCaregiverComing(true)} />}
...
function HomeView(props: any) {
  const { openChildMode, generated, setGenerated, situation, setSituation, format, setFormat, difficulty, setDifficulty, pendingRequest, setActive } = props
