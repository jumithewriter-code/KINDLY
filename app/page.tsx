  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav aria-label="Main navigation">{navItems.map(({ label, icon: Icon }) => <button key={label} className={active === label ? 'nav-item active' : 'nav-item'} onClick={() => setActive(label)}>{active === label ? null : <Icon size={19} />}<span>{label}</span>{label === 'Requests' && bathroomPending && <b className="nav-badge">1</b>}</button>)}</nav>
