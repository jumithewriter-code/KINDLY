    ...
    <main className="onboarding-page">
      ...
      <div className="onboarding-layout">
        ...
        <section className="onboarding-card" aria-labelledby="onboarding-title">
          ...
          {step === 2 && <div className="onboarding-form onboarding-complete"><div className="onboarding-check"><Check size={31} /></div><span className="eyebrow">YOU&apos;RE ALL SET</span><h2 id="onboarding-title">Welcome to your Kindly space, {name || 'friend'}.</h2><p className="onboarding-copy">We&apos;ll start with your chosen supports and learn what helps along the way.</p><a className="button coral" href="/">Go to my space <ArrowRight size={17} /></a></div>}
          {step < 2 && <div className="onboarding-actions"><button type="button" className="onboarding-back" onClick={() => setStep((current) => Math.max(0, current - 1))} disabled={step === 0}><ArrowLeft size={16} /> Previous</button><button type="button" className="button coral" onClick={() => setStep((current) => current + 1)}>{step === 0 ? 'Continue' : 'Finish setup'} <ArrowRight size={17} /></button></div>}
        </section>
      </div>
    </main>
  )
