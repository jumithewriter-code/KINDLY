  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav aria-label="Main navigation">{navItems.map(({ label, icon: Icon }) => <button key={label} className={active === label ? 'nav-item active' : 'nav-item'} onClick={() => setActive(label)}><Icon size={19} /><span>{label}</span>{label === 'Requests' && bathroomPending && <b className="nav-badge">1</b>}</button>)}</nav>
        <div className="sidebar-bottom"><button className="nav-item"><Settings2 size={19} /><span>Settings</span></button><div className="made-for"><Sparkles size={16} /><span>Made for<br /><strong>more good days</strong></span></div></div>
      </aside>
...
function HomeView(props: any) {
  ...
  return <div className="content-wrap">
    ...
    <div className="tool-grid"><button className="tool-card peach" onClick={() => document.getElementById('prepare')?.scrollIntoView({ behavior: 'smooth' })}><div className="tool-art">✦</div><b>Prepare for a situation</b><span>Make a simple plan together</span><ArrowRight size={17} /></button><button className="tool-card lavender" onClick={openChildMode}><div className="tool-art">☁</div><b>Practice communication</b><span>Try words, pictures, or gestures</span><ArrowRight size={17} /></button><button className="tool-card mint" onClick={() => setActive('Routines')}><div className="tool-art">☼</div><b>Build a routine</b><span>Make the next step clearer</span><ArrowRight size={17} /></button></div>
